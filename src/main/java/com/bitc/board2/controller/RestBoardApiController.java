...
//@ResponseBody : 클라이언트 요청에 대한 응답을 테이터 자체로 전달하는 어노테이션, 해당 어노테리션을 클래스위에 입력할 경우 해당
//                클래스의 멤버 전체에 효과가 있고 메서드 위에 입력할 경우 해당 메서드에만 적용됨
//JSON(JavaScript Object Notation) : 데이터 통신 시 사용하는 데이터 타입, Key와 Value이 1:1로 매칭되어 있는 데이터 타입
//rest api방식은 클라이언트의 종류에 상관없이 데이터 통신이 가능하게 해줌
@RestController
